...
def create_charts(df):
    ...
    # Sentiment Analysis Distribution (Logistic Regression vs Lexicon)
    styled_subheader(
        'Sentiment Analysis Distribution (Logistic Regression vs Lexicon)')
    ...
    # Pie Chart for Sentiment Distribution (Logistic Regression vs Lexicon)
    styled_subheader('Sentiment Distribution (Logistic Regression vs Lexicon)')
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    # Logistic Regression
    sentiment_counts_lr = df['sentiment_lr'].value_counts()
...
# Top 10 Reviews for each sentiment (Logistic Regression vs Lexicon)
    styled_subheader('Top 10 Reviews (Logistic Regression vs Lexicon)')
    ...
    for sentiment in ['positive', 'negative', 'neutral']:
        st.subheader(sentiment.capitalize() + ' Reviews')
        col1, col2 = st.columns(2)

        with col1:
            st.write(f"**Logistic Regression:**")
            st.dataframe(top_reviews_lr[sentiment]
                         [['Komentar', 'sentiment_lr']])

        with col2:
            st.write(f"**Lexicon Based:**")
            st.dataframe(top_reviews_lexicon[sentiment][[
                         'Komentar', 'sentiment_lexicon']])
